...
async function showApp(session){currentSession=session;cloudReady=false;try{await loadCloud();startRealtime();document.body.classList.remove('auth-locked');loginScreen.classList.add('hidden');appShell.hidden=false;logoutBtn.classList.remove('hidden');signedInUser.textContent=session.user.email||'';resetOwn();resetCom();renderAll()}catch(err){showLogin(err.message||'Unable to load GTL cloud data.')}}
...
async function initAuth(){passwordLoginBtn.onclick=passwordSignIn;loginPassword.addEventListener('keydown',e=>{if(e.key==='Enter')passwordSignIn()});logoutBtn.onclick=async()=>{await stopRealtime();await sb.auth.signOut();currentSession=null;cloudReady=false;showLogin('Signed out.')};const {data:{session}}=await sb.auth.getSession();if(session)await showApp(session);else showLogin();sb.auth.onAuthStateChange(async(event,session)=>{if(event==='SIGNED_IN'&&session&&session.access_token!==currentSession?.access_token)await showApp(session);if(event==='SIGNED_OUT'){await stopRealtime();currentSession=null;cloudReady=false;showLogin()}})}
