...
ownForm.onsubmit=e=>{e.preventDefault();syncPodInvoiceStatus('own');const edit=ownEditId.value;const r={id:edit||useId('own'),date:ownDate.value,truck:'KA07AD0725',from:ownFrom.value.trim(),to:ownTo.value.trim(),customerId:ownCustomer.value,otherCustomerName:ownCustomer.value==='other'?ownOtherCustomer.value.trim():'',freight:+ownFreight.value||0,paymentReceivedBy:ownPaymentBy.value.trim(),pod:ownPod.value,invoiceApplicable:ownInvoiceApplicable.value,invoiceGenerated:ownInvoiceGenerated.value,invoiceNumber:ownInvoiceNumber.value.trim(),remarks:ownRemarks.value.trim(),updatedAt:new Date().toISOString()};if(edit){db.ownTrips[db.ownTrips.findIndex(x=>x.id===edit)]=r}else db.ownTrips.push(r);save();resetOwn()};

function calcCommission(){const paying=+comPaying.value||0,paid=+comPaid.value||0,wo=+comWoGst.value||0;const pending=paying-paid;const commission=comCustomer.value==='cust-vahini'?.10*(wo-paying):0;const net=wo-paying-commission;comPending.value=pending.toFixed(2);comCommission.value=commission.toFixed(2);comNet.value=net.toFixed(2)}
document.querySelectorAll('.calc').forEach(x=>x.addEventListener('input',calcCommission));
commissionForm.onsubmit=e=>{e.preventDefault();syncPodInvoiceStatus('com');calcCommission();const edit=comEditId.value,paying=+comPaying.value||0,paid=+comPaid.value||0,wo=+comWoGst.value||0,commission=comCustomer.value==='cust-vahini'?.10*(wo-paying):0;const r={id:edit||useId('commission'),date:comDate.value,truck:comTruck.value.trim().toUpperCase(),driver:comDriver.value.trim(),from:comFrom.value.trim(),to:comTo.value.trim(),customerId:comCustomer.value,otherCustomerName:comCustomer.value==='other'?comOtherCustomer.value.trim():'',payingToDriver:paying,paidSoFar:paid,pending:paying-paid,customerWoGst:wo,customerWGst:+comWGst.value||0,commission,netIncome:wo-paying-commission,pod:comPod.value,invoiceApplicable:comInvoiceApplicable.value,invoiceGenerated:comInvoiceGenerated.value,invoiceNumber:comInvoiceNumber.value.trim(),remarks:comRemarks.value.trim(),updatedAt:new Date().toISOString()};if(edit){db.commissionTrips[db.commissionTrips.findIndex(x=>x.id===edit)]=r}else db.commissionTrips.push(r);save();resetCom()};
